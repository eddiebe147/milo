 ...
 *
 * Refactored from ClaudeClient to implement the AIProvider interface.
...
export class ClaudeProvider implements AIProvider {
  ...
  private model: string = DEFAULT_MODELS.claude
  ...
  async generateMorningBriefing(input: MorningBriefingInput): Promise<MorningBriefingOutput> {
    if (!this.client) {
      throw new Error('Claude provider not initialized. Please set your API key.')
    }
...
`
  ...
  private parseJsonResponse<T>(text: string, fallback: T): T {
    try {
      ...
      console.error('[ClaudeProvider] Failed to parse JSON response:', error)
      console.error('[ClaudeProvider] Raw response:', text)
      return fallback
    }
  }
}
